"""Soft validation endpoints for marketing records."""
...
from app.services.validation.soft_validation_service import SoftValidationService, marketing_models
...
@router.post(
    ...
    dependencies=[Depends(Require("marketing:write"))],
)
def validate_marketing_records(
    payload: SoftValidationRequest,
    db: Session = Depends(get_db),
) -> Dict[str, object]:
    """Run soft validation for marketing records and persist issues."""
...
@router.get(
    "/validation/marketing/issues",
    response_model=ValidationIssueList,
    dependencies=[Depends(Require("marketing:read"))],
)
def list_marketing_validation_issues(
    ...
    query = db.query(FinanceValidationIssue).filter(FinanceValidationIssue.scope == "marketing")
    if model:
...
@router.get(
    "/validation/marketing/issues/summary",
    response_model=ValidationSummaryResponse,
    dependencies=[Depends(Require("marketing:read"))],
)
def marketing_validation_summary(
    db: Session = Depends(get_db),
) -> ValidationSummaryResponse:
    query = db.query(FinanceValidationIssue).filter(FinanceValidationIssue.scope == "marketing")
