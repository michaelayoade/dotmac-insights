...
Single source of truth for all contact data in the CRM:
- Leads (pre-sales prospects)
- Prospects (qualified leads)
- Customers (paying customers)
- Churned (former customers)
- Persons (individual contacts at organizations)
...
class Contact(Base):
    """
    Contact record - single source of truth for all contact data.

    Contact Type Guidelines:
    ========================
    - LEAD: Pre-sale prospect
    ...
    - PERSON: Individual contact AT an organization
    ...
    child_contacts: Mapped[List["Contact"]] = relationship(
        "Contact",
        back_populates="parent",
        foreign_keys="[Contact.parent_id]"
    )
    parent: Mapped[Optional["Contact"]] = relationship(
        "Contact",
    ...
    __table_args__ = (
        ...
        Index("ix_contacts_type_status", "contact_type", "status"),
        Index("ix_contacts_email_type", "email", "contact_type"),
        Index("ix_contacts_phone_type", "phone", "contact_type"),
        Index("ix_contacts_owner_type", "owner_id", "contact_type"),
        Index("ix_contacts_territory", "territory"),
    ...
    def __repr__(self) -> str:
        return f"<Contact {self.id}: {self.name} ({self.contact_type.value})>"
    ...
    def display_name(self) -> str:
        """Get best display name for UI.

        For persons: full_name (first + last, or name if no parts)
        For organizations: company_name or name
        """
