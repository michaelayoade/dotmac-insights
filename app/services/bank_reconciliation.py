...
class BankReconciliationService:
    ...
    def get_outstanding_items(
        ...
        unmatched_bank_txns = self.db.query(BankTransaction).filter(
            and_(
        ...
        unmatched_gl = self.db.query(GLEntry).filter(
            and_(
        ...
        ).order_by(GLEntry.posting_date).all()
        ...
        for bank_id, bank_txn in bank_txns.items():
            ...
            for gl in gl_entries:
                ...
                if abs(bank_amount - gl_amount) < 0.02:
                    try:
                        self.match_transaction(bank_id, [gl["id"]], user_id)
                        matched.append({
