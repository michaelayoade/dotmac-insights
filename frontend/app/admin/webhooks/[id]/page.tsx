  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/webhooks" className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-elevated text-foreground hover:bg-slate-border">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-slate-card border border-slate-border rounded-xl p-5 lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Webhook Details</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                className={cn(
                  'inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors',
                  rotating ? 'bg-slate-elevated text-slate-muted' : 'bg-slate-elevated text-foreground hover:bg-slate-border'
                )}
              >
              ...
              <Button
                ...
                className={cn(
                  'inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors',
                  testing ? 'bg-slate-elevated text-slate-muted' : 'bg-slate-elevated text-foreground hover:bg-slate-border'
                )}
              >
        ...
        <div className="bg-slate-card border border-slate-border rounded-xl p-5 space-y-3">
          ...
          <div className="space-y-2">
            {(deliveries || []).slice(0, 5).map((delivery) => (
              ...
              >
                <div>
                  <p className="text-sm text-foreground">{delivery.event_type || 'event'}</p>
                  <p className="text-xs text-slate-muted">
      ...
      <div id="deliveries" className="bg-slate-card border border-slate-border rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-border flex items-center justify-between">
          ...
          <Button onClick={() => mutateDeliveries()} className="text-xs text-slate-muted hover:text-foreground inline-flex items-center gap-1">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-border">
            {(deliveries || []).map((delivery) => (
              <tr key={delivery.id}>
                <td className="px-4 py-3 text-foreground">{delivery.event_type || 'event'}</td>
          ...
          <div className="bg-slate-card border border-slate-border rounded-xl max-w-4xl w-full max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-border">
              <div>
                ...
                <p className="text-foreground text-sm">Delivery ID: {payloadView.id}</p>
              </div>
              <Button onClick={() => setPayloadView(null)} className="text-slate-muted hover:text-foreground">
                <X className="w-5 h-5" />
              </Button>
            </div>
