    ...
    { key: 'type', header: 'Type', render: (item: any) => <span className="text-slate-muted text-sm capitalize">{item.transaction_type || '-'}</span> },
    {
      ...
      render: (item: any) => (
        <div className="text-right">
          <span className="font-mono text-foreground">{formatAccountingCurrency(item.amount ?? item.deposit ?? item.withdrawal, item.currency)}</span>
          <div className="text-xs text-slate-muted">
            {item.deposit ? `+${formatAccountingCurrency(item.deposit, item.currency)}` : null}
            {item.withdrawal ? `-${formatAccountingCurrency(item.withdrawal, item.currency)}` : null}
          </div>
        </div>
      ),
    },
    {
      ...
      render: (item: any) => (
        <span className="font-mono text-slate-200">
          {formatAccountingCurrency(item.allocated_amount ?? 0, item.currency)} / {formatAccountingCurrency(item.amount ?? item.deposit ?? 0, item.currency)}
        </span>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <LinkButton href="/books/bank-transactions/new" variant="secondary" icon={Plus}>
            New Transaction
          </LinkButton>
          <LinkButton href="/books/bank-transactions/import" module="books" icon={Upload}>
            Import
          </LinkButton>
        </div>
      </div>
