...
import { Plus, Filter, FileText, Calendar, User } from 'lucide-react';
  ...
  const columns = [
    {
      ...
      render: (item: any) => (
        <div className="flex flex-col">
          <span className="font-mono text-white">{item.invoice_number || `#${item.id}`}</span>
          <span className="text-slate-muted text-sm">{formatDate(item.invoice_date)}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (item: any) => (
        <div className="text-right">
          ...
          <div className="text-xs text-slate-muted">Paid: {formatCurrency(item.amount_paid ?? 0, item.currency)}</div>
        </div>
      ),
    },
    {
