...
export default function NewBillPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/books/accounts-payable/bills');
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-slate-muted">New Bill</p>
            <h1 className="text-xl font-semibold text-foreground">Create Bill</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-4">
          ...
          {showMoreOptions && (
            <div className="space-y-4 pt-2 border-t border-slate-border/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="Bill Number" name="bill_number" value={form.bill_number} onChange={handleChange} placeholder="Supplier's invoice number" />
                <div>
