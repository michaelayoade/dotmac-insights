...
export default function BooksControlsPage() {
  ...
  const { data: approvals } = useAccountingPendingApprovals();
  const { data: workflows } = useAccountingWorkflows();
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Books Controls</h1>
          <p className="text-slate-muted text-sm">Period close, approvals, FX, tax, banking, and dunning operations.</p>
        </div>
      </header>
        ...
        <Card title="Period Close" icon={ShieldCheck}>
          <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {p.status === 'closed' ? (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg border border-slate-border text-white text-sm hover:bg-slate-elevated"
                      >
                        <Unlock className="w-4 h-4 inline" /> Reopen
                      </button>
                    ) : (
                      <button
                        onClick={() => closePeriod(p.id)}
                        className="px-3 py-1.5 rounded-lg bg-teal-electric text-slate-950 text-sm font-semibold hover:bg-teal-electric/90"
                      >
                        <Lock className="w-4 h-4 inline" /> Close
                      </button>
                    )}
                    <button
                      onClick={() => generateClosingEntries(p.id)}
                      className="px-3 py-1.5 rounded-lg border border-slate-border text-white text-sm hover:bg-slate-elevated"
                    >
                      Run closing
                    </button>
            ...
            <div className="flex flex-wrap gap-2 items-end">
              ...
              <button
                onClick={() => createPeriods({ fiscal_year: periodForm.fiscal_year, frequency: periodForm.frequency })}
        ...
        <Card title="FX Revaluation" icon={RefreshCcw}>
          ...
          <div className="flex gap-2 mt-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-border text-white text-sm hover:bg-slate-elevated"
            >
              Preview
            </button>
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title="Dunning & Credit Control" icon={Bell}>
          ...
          <div className="mt-3 flex gap-2">
            <button className="px-3 py-2 rounded-lg bg-teal-electric text-slate-950 text-sm font-semibold hover:bg-teal-electric/90">
              Send reminders
            </button>
            <button className="px-3 py-2 rounded-lg border border-slate-border text-white text-sm hover:bg-slate-elevated">
              Review holds
            </button>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title="Accounting Controls" icon={ShieldCheck}>
          <div className="space-y-2">
            ...
            <button
              onClick={() => updateControls(controlForm)}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-teal-electric text-slate-950 text-sm font-semibold hover:bg-teal-electric/90"
            >
              <RefreshCcw className="w-4 h-4" />
              Update controls
