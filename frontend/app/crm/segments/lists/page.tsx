...
import { LoadingState, Button, FilterCard, PageHeader } from '@/components/ui';
...
export default function CRMListsPage() {
  ...
  const { data, isLoading, error, mutate } = useSWR<ContactListsResponse>(
    canFetch ? `/contacts/lists?favorites_only=${filter === 'favorites'}` : null,
    apiFetch
  );
  ...
  if (missingScope) {
    return (
      <AccessDenied
        message="You need the crm:read permission to view lists."
        backHref="/crm"
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Tabs */}
      <FilterCard title="Filter Lists" contentClassName="flex items-center gap-2" iconClassName="text-indigo-400">
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {lists.map((list) => {
          ...
          const href = `/crm/contacts/all?${params.toString()}`;
      ...
      {/* Quick Links */}
      <FilterCard title="Quick Filters" contentClassName="flex flex-wrap gap-2" iconClassName="text-emerald-400">
        <Link
          href="/crm/contacts/all?type=customer"
        ...
        <Link
          href="/crm/contacts/all?type=lead"
        ...
        <Link
          href="/crm/contacts/all?status=active"
        ...
        <Link
          href="/crm/contacts/all?has_outstanding=true"
