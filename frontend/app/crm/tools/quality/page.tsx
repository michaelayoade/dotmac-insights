...
import { LoadingState, Button, PageHeader } from '@/components/ui';
...
export default function CRMQualityPage() {
  ...
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError, mutate: mutateAnalytics } = useSWR<QualityAnalytics>(
    canFetch ? '/contacts/analytics/quality' : null,
    apiFetch
  );
  ...
  if (missingScope) {
    return (
      <AccessDenied
        message="You need the crm:read permission to view data quality."
        backHref="/crm"
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="bg-slate-card rounded-xl border border-slate-border p-6 text-center">
            ...
            <Link
              href={filterParam ? `/crm/contacts/all?quality_issue=${filterParam}` : '/crm/contacts/all'}
      ...
      <div className="bg-slate-card rounded-xl border border-slate-border p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-slate-elevated rounded-lg">
            ...
            <Link
              href="/crm/contacts/all?missing=email"
