...
export default function CustomersPage() {
  ...
  const { data, isLoading, error, mutate } = useUnifiedContactCustomers(params, { isPaused: () => !canFetch });
  ...
  if (missingScope) {
    return (
      <AccessDenied
        message="You need the crm:read permission to view customers."
        backHref="/crm"
    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          canWrite ? (
            <Link
              href="/crm/contacts/new?type=customer"
      ...
      {/* Filters */}
      <FilterCard
        actions={(search || statusFilter || categoryFilter || hasOutstanding) && (
          <Button
            onClick={() => {
              setSearch('');
              setSearchInput('');
      ...
      >
        <form onSubmit={handleSearch} className="flex-1 min-w-[200px] max-w-md relative">
          ...
          <FilterInput
            ...
            className="w-full pl-10 pr-4 placeholder:text-slate-muted focus:ring-2 focus:ring-emerald-500/50"
          />
        </form>
        <FilterSelect
          value={statusFilter}
          onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
          className="focus:ring-2 focus:ring-emerald-500/50"
        >
