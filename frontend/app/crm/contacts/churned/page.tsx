...
import { Button, FilterCard, FilterInput, LoadingState, PageHeader } from '@/components/ui';
...
export default function ChurnedPage() {
  ...
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useSWR<ChurnAnalytics>(
    canFetch ? `/contacts/analytics/churn?period_days=${periodDays}` : null,
    apiFetch
  );
  ...
  if (missingScope) {
    return (
      <AccessDenied
        message="You need the crm:read permission to view churned customers."
        backHref="/crm"
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item: UnifiedContact) => (
        <span className="font-mono text-red-400">{formatCurrency(item.mrr ?? 0)}</span>
      ),
    },
    {
      key: 'territory',
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Reason Quick Filter */}
      <FilterCard title="Filter by Reason" iconClassName="text-red-400" contentClassName="flex flex-wrap gap-2">
        <Button
      ...
      >
        <form onSubmit={handleSearch} className="flex-1 min-w-[200px] max-w-md relative">
          ...
          <FilterInput
            ...
            className="w-full pl-10 pr-4 placeholder:text-slate-muted focus:ring-2 focus:ring-red-500/50"
          />
        </form>
      </FilterCard>
