...
export default function EditContactPage() {
  ...
  const { hasAccess, isLoading: authLoading } = useRequireScope('crm:write');
  const { hasAnyScope } = useAuth();
  const canHardDelete = hasAnyScope(['admin:write', '*']);

  const canFetch = hasAccess && !authLoading;
  const { data: contactData, isLoading: isLoadingContact, error: loadError } = useUnifiedContact(
    contactId,
    { isPaused: () => !canFetch }
  );
  ...
  if (!hasAccess) {
    return (
      <AccessDenied
        message="You need crm:write permission to edit contacts."
        backHref={`/crm/contacts/${contactId}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-foreground-secondary mb-2">Contact Type</label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {formData.is_organization ? (
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-foreground-secondary mb-2">Organization Name *</label>
                <input
                  type="text"
                  value={formData.name}
                  onChange={(e) => updateField('name', e.target.value)}
                  placeholder="Enter organization name"
                  className={`w-full px-3 py-2 rounded-lg bg-slate-900/50 border ${validationErrors.name ? 'border-red-500' : 'border-slate-600'} text-foreground placeholder-slate-500 focus:outline-none focus:border-cyan-500`}
                />
              ...
              <>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-foreground-secondary mb-2">Display Name *</label>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => updateField('name', e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded-lg bg-slate-900/50 border ${validationErrors.email ? 'border-red-500' : 'border-slate-600'} text-foreground placeholder-slate-500 focus:outline-none focus:border-cyan-500`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded-lg bg-slate-900/50 border ${validationErrors.phone ? 'border-red-500' : 'border-slate-600'} text-foreground placeholder-slate-500 focus:outline-none focus:border-cyan-500`}
              />
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-foreground-secondary mb-2">Address Line 1</label>
              <input
                type="text"
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {formData.tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-300 text-sm">
                {tag}
                <Button type="button" onClick={() => removeTag(tag)} className="hover:text-foreground transition-colors">&times;</Button>
              </span>
            ))}
            {formData.tags.length === 0 && <span className="text-slate-500 text-sm">No tags added</span>}
          </div>
          ...
          <div className="flex gap-2">
            ...
            />
            <Button type="button" onClick={addTag} className="px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-foreground transition-colors">
              Add
            </Button>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-6 mt-8">
          ...
          <p className="text-slate-400 text-sm mb-4">Actions here cannot be easily undone.</p>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-lg bg-slate-800/50 border border-slate-700">
              <div>
                ...
                <p className="text-slate-400 text-sm">Mark this contact as inactive.</p>
              </div>
              <Button
                type="button"
        ...
        <div className="flex justify-between items-center">
          <div>{hasChanges && <span className="text-sm text-amber-400">You have unsaved changes</span>}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-slate-800 border border-slate-700 rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button onClick={() => setShowArchiveDialog(false)} className="p-1 rounded hover:bg-slate-700 transition-colors">
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {isDeleting ? (<><Loader2 className="w-4 h-4 animate-spin" />Archiving...</>) : (<><Archive className="w-4 h-4" />Archive</>)}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-slate-800 border border-red-500/30 rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button onClick={() => setShowDeleteDialog(false)} className="p-1 rounded hover:bg-slate-700 transition-colors">
                <X className="w-5 h-5 text-slate-400" />
              </Button>
            </div>
