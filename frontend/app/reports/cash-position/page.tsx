    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-card border border-slate-border rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-foreground">{summary.data?.accounts?.length || 0}</p>
        </div>
      </div>
      ...
      <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-foreground font-semibold">Balances by Account</h3>
        </div>
        <DataTable
          columns={[
            { key: 'account', header: 'Account', render: (row: any) => <span className="text-foreground">{row.account}</span> },
            { key: 'balance', header: 'Balance', align: 'right' as const, render: (row: any) => <span className="font-mono text-foreground">{formatCurrency(row.balance, summary.data?.currency || 'NGN')}</span> },
          ]}
      ...
      <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-foreground font-semibold">Forecast</h3>
        ...
        <DataTable
          columns={[
            ...
            { key: 'projected_cash', header: 'Projected Cash', align: 'right' as const, render: (row: any) => <span className="font-mono text-foreground">{formatCurrency(row.projected_cash, summary.data?.currency || 'NGN')}</span> },
          ]}
          data={forecast.data || []}
          keyField="period"
