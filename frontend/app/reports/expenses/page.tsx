  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold text-foreground">Expenses Reports</h1>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {summary.data?.categories?.slice(0, 3).map((cat: any) => (
          <div key={cat.category} className="bg-slate-card border border-slate-border rounded-xl p-4">
            ...
            <p className="text-xl font-bold text-foreground">{formatCurrency(cat.total, summary.data?.currency || 'NGN')}</p>
      ...
      <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-foreground font-semibold">Expense Trend</h3>
        ...
        <DataTable
          columns={[
            ...
            { key: 'total', header: 'Total', align: 'right' as const, render: (row: any) => <span className="font-mono text-foreground">{formatCurrency(row.total, summary.data?.currency || 'NGN')}</span> },
          ]}
          data={trend.data || []}
          keyField="period"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          ...
          <DataTable
            columns={[
              { key: 'category', header: 'Category', render: (row: any) => <span className="text-foreground">{row.category}</span> },
              { key: 'total', header: 'Total', align: 'right' as const, render: (row: any) => <span className="font-mono text-foreground">{formatCurrency(row.total, summary.data?.currency || 'NGN')}</span> },
        ...
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-foreground font-semibold">By Vendor</h3>
          </div>
          <DataTable
            columns={[
              { key: 'vendor', header: 'Vendor', render: (row: any) => <span className="text-foreground">{row.vendor || '-'}</span> },
              { key: 'total', header: 'Total', align: 'right' as const, render: (row: any) => <span className="font-mono text-foreground">{formatCurrency(row.total, summary.data?.currency || 'NGN')}</span> },
