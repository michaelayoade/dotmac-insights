...
export default function TagsPage() {
  ...
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError, mutate: mutateAnalytics } = useSWR<TagsAnalytics>(
    '/contacts/analytics/tags?limit=100',
    apiFetch
  );
  ...
  const {
    ...
    selectedTag ? `/contacts?tag=${encodeURIComponent(selectedTag)}&page=1&page_size=100` : null,
    apiFetch
  );
      ...
      {selectedTag && (
        <div className="bg-slate-card rounded-xl border border-slate-border p-4">
          ...
          <div className="space-y-2">
            ...
            {contacts.length > 10 && (
              <Link
                href={`/contacts?tag=${encodeURIComponent(selectedTag)}`}
            ...
            {filteredTags.map((tagData, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link
                    href={`/contacts?tag=${encodeURIComponent(tagData.tag)}`}
