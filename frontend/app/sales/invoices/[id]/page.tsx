      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-semibold text-foreground">{data.invoice_number || `Invoice #${data.id}`}</h1>
          </div>
        </div>
      ...
      {data.payments?.length ? (
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-foreground font-semibold">Payments</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-slate-muted">
              ...
              <tbody>
                {data.payments.map((p: any) => (
                  <tr key={p.id} className="border-t border-slate-border/60">
                    ...
                    <td className="px-2 py-2 text-right text-foreground font-mono">{formatCurrency(p.amount, data.currency || 'NGN')}</td>
      ...
      {(data as any).credit_notes?.length ? (
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-foreground font-semibold">Credit Notes</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-slate-muted">
              ...
              <tbody>
                {(data as any).credit_notes.map((c: any) => (
                  <tr key={c.id} className="border-t border-slate-border/60">
                    <td className="px-2 py-2 text-foreground font-mono">{c.credit_number || `CN-${c.id}`}</td>
                    <td className="px-2 py-2 text-right text-foreground font-mono">{formatCurrency(c.amount, data.currency || 'NGN')}</td>
