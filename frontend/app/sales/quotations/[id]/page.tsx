...
import { AlertTriangle, ArrowLeft, FileText, StickyNote } from 'lucide-react';
import { useFinanceQuotationDetail } from '@/hooks/useApi';
...
export default function SalesQuotationDetailPage() {
  ...
  const { data, isLoading, error } = useFinanceQuotationDetail(Number.isFinite(id) ? id : null, 'NGN');
  ...
  const summary = [
    { label: 'Quotation #', value: data.quotation_number || data.erpnext_id || `QTN-${data.id}` },
    { label: 'Customer', value: data.customer_name || data.party_name || (data.customer_id ? `Customer #${data.customer_id}` : '-') },
    { label: 'Status', value: data.status || 'draft' },
    { label: 'Date', value: formatDate((data as any).transaction_date || data.quotation_date) },
    { label: 'Valid Till', value: formatDate((data as any).valid_till) },
    {
      label: 'Total',
