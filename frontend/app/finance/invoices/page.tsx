...
export default function InvoicesPage() {
  ...
  const [sortBy, setSortBy] = useState<'invoice_date' | 'due_date' | 'total_amount' | 'amount_paid' | 'customer_id' | 'status'>('invoice_date');
    ...
    },
    {
      ...
      render: (item: any) =>
        item.credit_note_number ? (
          <span className="text-xs text-amber-warn font-mono">{item.credit_note_number}</span>
        ) : (
      ...
      render: (item: any) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <span className={cn(isOverdue ? 'text-red-400' : 'text-slate-muted')}>
              {formatDate(item.due_date)}
            </span>
            {isOverdue && (
              <span className="text-xs text-red-400">({item.days_overdue}d)</span>
            )}
          </div>
        );
      },
    },
    {
      ...
      render: (item: any) => (
        <div className="flex items-center gap-2 text-xs">
          <Link
            href={`/finance/payments?invoice_id=${item.id}`}
          ...
          <Link
            href={`/finance/credit-notes?invoice_id=${item.id}`}
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        onRowClick={(item) => router.push(`/finance/invoices/${(item as any).id}`)}
