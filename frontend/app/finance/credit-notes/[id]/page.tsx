...
import { AlertTriangle, ArrowLeft, Receipt, FileText } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-muted">
        ...
        <span>/</span>
        <Link href="/finance/credit-notes" className="hover:text-white">Credit Notes</Link>
        <span>/</span>
        <span className="text-white">{data.credit_note_number || `#${data.id}`}</span>
      </div>
      ...
      <div className="bg-slate-card rounded-xl border border-slate-border p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              {data.credit_note_number || `#${data.id}`}
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-muted">
          <div>Issue Date: <span className="text-white">{formatDate(data.date)}</span></div>
          <div>Reason: <span className="text-white">{data.reason || '—'}</span></div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-muted">
          ...
          <div>
            ...
            <p className="text-white">
              Splynx: {data.external_ids?.splynx_id ?? '—'}
            </p>
          </div>
