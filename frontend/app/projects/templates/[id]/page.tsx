...
export default function TemplateDetailPage() {
  ...
  const templateId = typeof params.id === 'string' ? parseInt(params.id) : null;

  const { data: template, isLoading, error } = useProjectTemplate(templateId);
  ...
  useEffect(() => {
    if (template) {
      ...
      if (template.milestone_templates) {
        setMilestones(
          template.milestone_templates.map((m: MilestoneTemplateItem, idx: number) => ({
            _tempId: `m-${m.id || idx}`,
            _existingId: m.id,
            name: m.name,
      ...
      if (template.task_templates) {
        setTasks(
          template.task_templates.map((t: TaskTemplateItem, idx: number) => ({
            _tempId: `t-${t.id || idx}`,
            _existingId: t.id,
            subject: t.subject,
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: ProjectTemplateUpdatePayload = {
        name: name.trim(),
        ...
        milestone_templates: milestones
          ...
          .map(({ _tempId, _existingId, ...m }) => ({
            ...m,
            name: m.name.trim(),
          })),
        task_templates: tasks
          .filter((t) => t.subject.trim())
          .map(({ _tempId, _existingId, _milestoneTempId, ...t }) => ({
            ...t,
            subject: t.subject.trim(),
          })),
      };

      await mutations.update(templateId, payload);
      mutate(['project-template', templateId]);
  ...
  if (error || !template) {
    return (
      <div className="text-center py-20">
        <p className="text-rose-400 mb-4">Template not found</p>
        <Link href="/projects/templates" className="text-amber-400 hover:text-amber-300">
          Back to templates
        </Link>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">{template.name}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href={`/projects/new?template=${templateId}`}
            className="flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-100 font-medium rounded-lg transition-colors"
          >
        ...
        <div className="space-y-4">
          ...
          {milestones.length === 0 ? (
            <div className="text-center py-12 bg-slate-800/30 rounded-lg border border-slate-700/50">
              ...
              <p className="text-slate-400">No milestones in this template</p>
              <button
                onClick={addMilestone}
        ...
        <div className="space-y-4">
          ...
          {tasks.length === 0 ? (
            <div className="text-center py-12 bg-slate-800/30 rounded-lg border border-slate-700/50">
              ...
              <p className="text-slate-400">No tasks in this template</p>
              <button
                onClick={addTask}
                className="mt-4 text-amber-400 hover:text-amber-300 text-sm font-medium"
              >
