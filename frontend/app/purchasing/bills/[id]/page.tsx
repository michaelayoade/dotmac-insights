...
import { usePurchasingBillDetail, usePurchasingBillMutations } from '@/hooks/useApi';
...
export default function PurchasingBillDetailPage() {
  ...
  const { data, isLoading, error } = usePurchasingBillDetail(Number.isFinite(id) ? id : null);
  const { updateBill } = usePurchasingBillMutations();
  ...
  if (error || !data) {
    return (
      <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 text-center">
        ...
        <p className="text-red-400">Failed to load bill</p>
        <button
          onClick={() => router.back()}
  ...
  const summaryRows = [
    { label: 'Supplier', value: data.supplier_name || data.supplier || '-' },
    { label: 'Bill Date', value: formatDate(data.posting_date) },
    { label: 'Due Date', value: formatDate(data.due_date) },
    { label: 'Status', value: data.status },
    { label: 'Total', value: formatCurrency(data.grand_total, data.currency || 'NGN') },
    { label: 'Outstanding', value: formatCurrency(data.outstanding_amount, data.currency || 'NGN') },
  ];
  ...
  const handleQuickUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActionError(err?.message || 'Failed to update bill');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-slate-muted">Bill</p>
            <h1 className="text-xl font-semibold text-foreground">{data.erpnext_id || `Bill #${data.id}`}</h1>
      ...
      <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
        ...
        <form onSubmit={handleQuickUpdate} className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
      ...
      {data.gl_entries?.length ? (
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            <Building2 className="w-4 h-4 text-teal-electric" />
            <h3 className="text-foreground font-semibold">GL Entries</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-slate-muted">
              ...
              <tbody>
                {data.gl_entries.map((entry: any) => (
                  <tr key={entry.id} className="border-t border-slate-border/60">
                    <td className="px-2 py-2 text-foreground font-mono">{entry.account}</td>
