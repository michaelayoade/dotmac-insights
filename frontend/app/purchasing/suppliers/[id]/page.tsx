      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-semibold text-foreground">{data.name || (data as any).supplier_name || `Supplier #${data.id}`}</h1>
          </div>
        </div>
        <Link
      ...
      <div className="bg-slate-card border border-slate-border rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        {summary.map((row) => (
          <div key={row.label}>
            ...
            <p className={cn('text-foreground font-semibold break-all', row.label === 'Type' && 'capitalize')}>
              {row.value}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p className="text-foreground font-semibold">{formatCurrency(data.total_purchases)}</p>
            </div>
            <div>
              <p className="text-slate-muted text-xs uppercase tracking-[0.1em]">Outstanding</p>
              <p className="text-foreground font-semibold">{formatCurrency(data.total_outstanding)}</p>
            </div>
            <div>
              <p className="text-slate-muted text-xs uppercase tracking-[0.1em]">Bill Count</p>
              <p className="text-foreground font-semibold">{data.bill_count ?? 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-card border border-slate-border rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-foreground font-semibold">Recent Bills</h3>
          </div>
