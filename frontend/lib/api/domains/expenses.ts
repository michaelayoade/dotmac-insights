...
export const expensesApi = {
  ...
  getCashAdvanceDetail: (id: number) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}`),
  createCashAdvance: (payload: CashAdvanceCreatePayload) =>
    fetchApi<CashAdvance>('/expenses/cash-advances/', { method: 'POST', body: JSON.stringify(payload) }),
  submitCashAdvance: (id: number, company_code?: string) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}/submit`, { method: 'POST', params: company_code ? { company_code } : undefined }),
  approveCashAdvance: (id: number) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}/approve`, { method: 'POST' }),
  rejectCashAdvance: (id: number, reason: string) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}/reject`, { method: 'POST', params: { reason } }),
  disburseCashAdvance: (id: number, payload: CashAdvanceDisbursePayload) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}/disburse`, { method: 'POST', body: JSON.stringify(payload) }),
  settleCashAdvance: (id: number, payload: CashAdvanceSettlePayload) =>
    fetchApi<CashAdvance>(`/expenses/cash-advances/${id}/settle`, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  createCorporateCardTransaction: (payload: CorporateCardTransactionCreatePayload) =>
    fetchApi<CorporateCardTransaction>('/expenses/transactions/', { method: 'POST', body: JSON.stringify(payload) }),
  matchTransaction: (id: number, expenseClaimLineId: number, confidence?: number) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/match`, { method: 'POST', body: JSON.stringify({ expense_claim_line_id: expenseClaimLineId, confidence }) }),
  unmatchTransaction: (id: number) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/unmatch`, { method: 'POST' }),
  disputeTransaction: (id: number, reason: string) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/dispute`, { method: 'POST', body: JSON.stringify({ reason }) }),
  resolveTransactionDispute: (id: number, resolutionNotes: string, newStatus?: string) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/resolve`, { method: 'POST', params: { resolution_notes: resolutionNotes, new_status: newStatus } }),
  excludeTransaction: (id: number) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/exclude`, { method: 'POST' }),
  markTransactionPersonal: (id: number) =>
    fetchApi<CorporateCardTransaction>(`/expenses/transactions/${id}/mark-personal`, { method: 'POST' }),
  deleteTransaction: (id: number) =>
    fetchApi<void>(`/expenses/transactions/${id}`, { method: 'DELETE' }),
  ...
  createCorporateCardStatement: (payload: { card_id: number; period_start: string; period_end: string; statement_date?: string; import_source?: string; original_filename?: string }) =>
    fetchApi<CorporateCardStatement>('/expenses/statements/', { method: 'POST', body: JSON.stringify(payload) }),
  importStatement: (payload: StatementImportPayload) =>
    fetchApi<CorporateCardStatement>('/expenses/statements/import', { method: 'POST', body: JSON.stringify(payload) }),
  reconcileStatement: (id: number) =>
    fetchApi<CorporateCardStatement>(`/expenses/statements/${id}/reconcile`, { method: 'POST' }),
  closeStatement: (id: number) =>
    fetchApi<CorporateCardStatement>(`/expenses/statements/${id}/close`, { method: 'POST' }),
  reopenStatement: (id: number) =>
    fetchApi<CorporateCardStatement>(`/expenses/statements/${id}/reopen`, { method: 'POST' }),
  getStatementTransactions: (id: number, status?: string) =>
    fetchApi<CorporateCardTransaction[]>(`/expenses/statements/${id}/transactions`, { params: { status } }),
  deleteStatement: (id: number) =>
    fetchApi<void>(`/expenses/statements/${id}`, { method: 'DELETE' }),
