...
def test_metrics_route_has_no_database_or_business_work() -> None:
    ...
    imported_services = {
        ...
        if isinstance(node, ast.ImportFrom) and str(node.module).startswith("app.services")
    }
...
def test_prometheus_callbacks_stay_in_reviewed_exporter_modules() -> None:
    ...
    for path in (ROOT / "app").rglob("*.py"):
        ...
        if ("REGISTRY.register(" in source or ".set_function(" in source) and path not in reviewed:
